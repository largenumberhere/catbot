...
pub struct TokenLoader <TToken: TokenDetails + Sized + IntoEnumIterator + Eq + Hash + std::fmt::Debug>{
    tokens: HashMap<TToken, String>
}

impl<TToken: TokenDetails + Sized + IntoEnumIterator + Eq + Hash + std::fmt::Debug> TokenLoader<TToken>  {
    ///Give this your token enum. Panics at runtime if not all tokens can be loaded
...
impl<TToken : TokenDetails + Sized + IntoEnumIterator + Eq + Hash + std::fmt::Debug> TokenLoader<TToken> {
    ...
    pub fn get_unwrap(&self, token :&TToken) -> String{
        ...
        match value {
            Some(v) => v,
            None =>{
                panic!("No token key found for token key'{:#?}' This should not have happened!", token)
            }
        }
    }
}
